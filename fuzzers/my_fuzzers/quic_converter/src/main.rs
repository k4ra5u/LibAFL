...
struct Opt {
    ...
    #[arg(
        long,
        help = "TCP server address (当transport=tcp时使用)",
        name = "tcp-addr",
...
pub fn main() {
    ...
    // let mut quic_st = QuicStruct::new("myserver.xx".to_owned(), opt.port, opt.host.to_owned());
    ...
    // quic_struct_shmem.write_to_env("QUIC_STRUCT").unwrap();
    // let quic_shmem_buf = quic_struct_shmem.as_slice_mut();
    // quic_shmem_buf[0] = 0;
    // quic_shmem_buf[1..9].copy_from_slice( &0x0usize.to_be_bytes());
    // let mut exit_kind = ExitKind::Ok;
