...
fn main() {
    ...
    let mut quic_converter = start_quic_converter(&opt.first_port.to_string(),unsafe {SHMEM_QUIC_STRUCT.as_ref().unwrap().id().to_string()});
    ...
    loop {
        ...
        let total_bytes = edges_observer.usable_count();
        info!("count_bytes/total_bytes: {}/{}", count_bytes, total_bytes);
