...
pub fn main() {
    ...
    let mut executor = NetworkRestartExecutor::new(tuple_list!(time_observer, edges_observer),shmem_provider)
        .start_command("/home/john/Desktop/cjj_related/testing_new/fuzzing-test/LibAFL/fuzzers/my_fuzzers/my_UDP_fuzzer_test_with_aflplusplus/start.sh".to_owned())
        .judge_command("ps aux | grep udp_echo_server | grep -v grep | awk '{print $2}'".to_owned())
        .port(12345)
    ...
    //     .unwrap();
        // let mut executor = NetworkRestartExecutor::new("/home/john/Desktop/cjj_related/testing_new/fuzzing-test/LibAFL/fuzzers/my_fuzzers/my_UDP_fuzzer_test_with_aflplusplus/start.sh", "ps aux | grep udp_echo_server | awk '{print $2}' | xargs kill -9", vec![
        //     //("__AFL_SHM_ID".to_string(), unsafe { shmem_buf.as_ref().id().to_string() }),
        //     ("__AFL_SHM_SIZE".to_string(), 65536.to_string()),
        //     ("__AFL_SHM".to_string(), unsafe { SHMEM_EDGE_MAP.as_ref().unwrap().id().to_string() })
        // ], 12345, Duration::from_millis(100), tuple_list!(time_observer, edges_observer));
        // let mut executor = NetworkRestartExecutor::new("/home/john/Desktop/cjj_related/testing_new/fuzzing-test/LibAFL/fuzzers/my_fuzzers/my_UDP_fuzzer_test_with_aflplusplus/start.sh", "ps aux | grep udp_echo_server | grep -v grep | awk '{print $2}'", vec![
        //     //("__AFL_SHM_ID".to_string(), unsafe { shmem_buf.as_ref().id().to_string() }),
        //     ("__AFL_SHM_SIZE".to_string(), 65536.to_string()),
        //     ("__AFL_SHM".to_string(), unsafe { SHMEM_EDGE_MAP.as_ref().unwrap().id().to_string() })
        // ], 12345, Duration::from_millis(100), tuple_list!(time_observer, edges_observer));
